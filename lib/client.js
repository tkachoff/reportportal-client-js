...
class RPClient {
    ...
    sendFile(itemId, saveLogRQ) {
        ...
        rest.post(url, {
            multipart: true,
            data: {
                'json_request_part': [jsonData, ],
                'binary_part': rest.file('doug-e-fresh_the-show.mp3', null, null, null, null)
            }
        }, this.options).on('complete', function (data) {
            console.log(data);
        });
    }
